...
def UDK_Camera():
    ...
    selectedcam = bpy.context.object
    ...
    for marker in bpy.data.scenes['Scene'].timeline_markers:
        ...
        if st == 1:
            camera_endfr = marker.frame    
            st = 2
        
        if st == 0:
            if marker.camera == selectedcam:
    ...
    bpy.context.object.data.name = dumcamname
    bpy.data.objects[childo].name = dumcamname 
        
    bpy.ops.nla.bake(frame_start=camera_startfr, frame_end=camera_endfr, visual_keying=True, clear_constraints=True, clear_parents=True, use_current_action=True, bake_types={'OBJECT'})
            
        
    """------old_unreal_cam---------"""

    origname = bpy.context.active_object.name
    origin = bpy.data.objects[origname]
    orcamname = bpy.context.object.data.name
    ...
    bpy.context.object.data.name = "Unreal_" + origname
    bpy.data.objects[childo].name = "Unreal_" + origname
    unrlc = bpy.data.objects["Unreal_" + origname]
    uncamname = bpy.context.object.data.name

    bpy.data.cameras[uncamname].type = bpy.data.cameras[orcamname].type 
    bpy.data.cameras[uncamname].lens = bpy.data.cameras[orcamname].lens 
    bpy.data.cameras[uncamname].lens_unit = bpy.data.cameras[orcamname].lens_unit
    bpy.data.cameras[uncamname].shift_x = bpy.data.cameras[orcamname].shift_x
    bpy.data.cameras[uncamname].shift_y = bpy.data.cameras[orcamname].shift_y
    bpy.data.cameras[uncamname].clip_start = bpy.data.cameras[orcamname].clip_start
    bpy.data.cameras[uncamname].clip_end = bpy.data.cameras[orcamname].clip_end
    bpy.data.cameras[uncamname].sensor_width = bpy.data.cameras[orcamname].sensor_width
    bpy.data.cameras[uncamname].sensor_fit = bpy.data.cameras[orcamname].sensor_fit

        

    (start_frame, end_frame) = origin.animation_data.action.frame_range

    for fr in range(int(start_frame), int(end_frame) + 1):

        
        bpy.data.scenes[Scene_Name].frame_set(fr)
                
        opx = origin.location[0]
        opy = origin.location[1]
        opz = origin.location[2]

        orx = origin.rotation_euler[0]
        ory = origin.rotation_euler[1]
        orz = origin.rotation_euler[2]
        ...
        crx = orx
        cry = ory
        crz = orz + math.radians(90)

        unrlc.location[0] = cpx
        unrlc.keyframe_insert(data_path="location", frame=fr, index=0)
        unrlc.location[1] = cpy
        unrlc.keyframe_insert(data_path="location", frame=fr, index=1)
        unrlc.location[2] = cpz
        unrlc.keyframe_insert(data_path="location", frame=fr, index=2)

        unrlc.rotation_euler[0] = crx
        unrlc.keyframe_insert(data_path="rotation_euler", frame=fr, index=0)
        unrlc.rotation_euler[1] = cry
        unrlc.keyframe_insert(data_path="rotation_euler", frame=fr, index=1)
        unrlc.rotation_euler[2] = crz
        unrlc.keyframe_insert(data_path="rotation_euler", frame=fr, index=2)
    ...
    bpy.ops.export_scene.fbx(filepath = fn + ".fbx", global_scale=100, use_mesh_modifiers=True, mesh_smooth_type = 'OFF', use_selection=True, version='ASCII6100')
...
class UDKCameraOperator(bpy.types.Operator):
    """Adjust camera for exporting to Unreal Engine and Export to FBX"""
    bl_idname = "object.udkcam"
...
def FBX_Batch():
    ...
    for obj in selection:
        ...
        bpy.ops.export_scene.fbx(filepath = fn + ".fbx", global_scale=1000, use_mesh_modifiers=True, mesh_smooth_type = 'OFF', use_selection=True, version='BIN7400')
...
class YetiToolPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col.separator()
        col.operator("object.udkcam", icon="CAMERA_DATA")
...
def register():
    ...
    bpy.utils.register_class(UDKCameraOperator)
...
def unregister():
    ...
    bpy.utils.unregister_class(UDKCameraOperator)
